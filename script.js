            ...
            <div class="flip-card">
                <div class="flip-card-inner">
                    ...
                    <div class="flip-card-back rounded-3 px-2 py-4" style="background-image: url('${IMG_BASE_URL + movie.poster_path}');">
                        ...
                        <form action="history-insert.php" method="post" target="_blank">
                            <input type="hidden" name="movie_id" value="${movie.id}">
                            <input type="hidden" name="movie_trailer" value="${trailerUrl}">
                            <button class="btn btn-primary" type="submit" ${!trailerUrl ? 'disabled' : ''}>Watch Now</button>
                        </form>
                    </div>
                </div>
            </div>`;
...
async function fetchMovieById(id) {
    try {
        ...
        console.error("Movie fetch failed for ID:", id);
        return null;
    }
}
...
async function fetchHistoryMovies() {
    try {
        const res = await fetch("history-fetch.php");
        const movieIds = await res.json();
        if (movieIds.error) {
            console.error("Error:", movieIds.error);
            return;
        }
        const movies = [];
